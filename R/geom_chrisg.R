...
##' @export
draw_key_chrisg <- function(data, params, size) {
  filename <- system.file(paste0(data$chrisg, ".png"), package = "ggchrisg")
...
##' @param nudge_x horizontal adjustment to nudge Chris G
...
##' library("ggplot2")
##' ggplot(mtcars) +
##' ggchrisg::geom_chrisg(aes(mpg, wt), chrisg = "hagridhoodie", size = 5)
##'
##' set.seed(1)
##' df <- data.frame(
##' 	x = rnorm(10),
...
##'
geom_chrisg <- function(mapping = NULL, data = NULL, stat = "identity",
...
GeomChrisG <- ggplot2::ggproto("GeomChrisG", ggplot2::Geom,
  ...
  draw_panel = function(data, panel_params, coord, by, na.rm = FALSE,
    ...
    if (!is.null(.fun) && is.function(.fun)) {
      data$ca <- .fun(data$chrisg)
    }
    if (is.null(data$chrisg)) {
      return(NULL)
    }

    groups <- split(data, factor(data$chrisg))
    imgs <- names(groups)
    grobs <- lapply(seq_along(groups), function(i) {
      ...
      imageGrob(
        d$x, d$y, d$size / 5, imgs[i], by, hjust,
  ...
  non_missing_aes = c("size", "chrisg"),
  required_aes = c("x", "y"),
  draw_key = draw_key_chrisg ## draw_key_blank ## need to write the `draw_key_chrisg` function.
)
...
imageGrob <- function(x, y, size, img, by, hjust, colour, alpha, image_fun, angle, asp = 1) {
  if (!methods::is(img, "magick-image")) {
    filename <- system.file(paste0(img, ".png"), package = "ggchrisg")
